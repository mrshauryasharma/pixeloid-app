...
export default function Dashboard() {
  ...
  const planLimits: Record<string, { chats: string; color: string }> = { free: { chats: '20/day', color: '#667eea' }, weekly: { chats: '100/week', color: '#4facfe' }, monthly: { chats: '200/month', color: '#f093fb' }, yearly: { chats: 'Unlimited', color: '#f5576c' } };
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ background: 'linear-gradient(135deg, rgba(102,126,234,0.15), rgba(118,75,162,0.15))', backdropFilter: 'blur(30px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 'clamp(16px, 3vw, 28px)', padding: 'clamp(20px, 4vw, 40px)', marginBottom: 'clamp(20px, 3vw, 40px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div style={{ flex: '1 1 300px' }}>
            <h1 style={{ fontSize: 'clamp(24px, 5vw, 42px)', fontWeight: '900', background: 'linear-gradient(to right, #f093fb, #667eea, #4facfe)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', margin: '0 0 8px 0', letterSpacing: '-1px' }}>Welcome Back, {user?.displayName || user?.email?.split('@')[0] || 'User'}! 👋</h1>
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 'clamp(14px, 2vw, 18px)', margin: 0, fontWeight: '300' }}>Here's your Pixeloid dashboard overview</p>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: 'clamp(12px, 2vw, 16px) clamp(16px, 3vw, 28px)', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
            <div style={{ width: 'clamp(36px, 5vw, 48px)', height: 'clamp(36px, 5vw, 48px)', borderRadius: '50%', background: 'linear-gradient(135deg, #667eea, #764ba2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(18px, 3vw, 24px)', flexShrink: 0 }}>
              {user?.photoURL ? <img src={user.photoURL} alt="avatar" style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} /> : '👤'}
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ color: 'white', fontWeight: '700', margin: 0, fontSize: 'clamp(13px, 2vw, 16px)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.displayName || user?.email?.split('@')[0] || 'User'}</p>
              <p style={{ color: 'rgba(255,255,255,0.5)', margin: '4px 0 0 0', fontSize: 'clamp(11px, 1.5vw, 13px)' }}>{user?.email || ''}</p>
            </div>
          </div>
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'clamp(12px, 2vw, 24px)', marginBottom: 'clamp(20px, 3vw, 40px)' }}>
          {[ 
            ...
            <motion.div key={index} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + index * 0.1 }} whileHover={{ y: -6 }} style={{ background: stat.gradient, backdropFilter: 'blur(20px)', border: `1px solid ${stat.color}20`, borderRadius: 'clamp(16px, 3vw, 24px)', padding: 'clamp(20px, 3vw, 32px)', transition: 'all 0.3s', cursor: 'default', position: 'relative', overflow: 'hidden' }}>
              ...
              <h3 style={{ color: 'rgba(255,255,255,0.5)', fontSize: 'clamp(11px, 1.5vw, 13px)', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '8px', fontWeight: '600' }}>{stat.title}</h3>
              <p style={{ color: stat.color, fontSize: 'clamp(28px, 5vw, 42px)', fontWeight: '900', margin: '0 0 4px 0', lineHeight: '1' }}>{stat.value}</p>
              {stat.subtitle && <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 'clamp(11px, 1.5vw, 13px)', margin: 0 }}>{stat.subtitle}</p>}
            </motion.div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(16px, 2vw, 24px)' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} style={{ background: 'linear-gradient(135deg, rgba(240,147,251,0.15), rgba(102,126,234,0.15))', backdropFilter: 'blur(20px)', border: '1px solid rgba(240,147,251,0.3)', borderRadius: 'clamp(16px, 3vw, 24px)', padding: 'clamp(20px, 3vw, 36px)', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
            <div style={{ fontSize: 'clamp(40px, 8vw, 60px)', marginBottom: '16px' }}>👑</div>
            <h3 style={{ color: 'white', fontSize: 'clamp(20px, 3vw, 24px)', fontWeight: '700', marginBottom: '8px' }}>Go Premium</h3>
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 'clamp(12px, 1.8vw, 14px)', marginBottom: 'clamp(16px, 2vw, 24px)', lineHeight: '1.6' }}>Unlock unlimited AI chats and more!</p>
            <motion.a href="/pricing" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} style={{ background: 'linear-gradient(135deg, #667eea, #f093fb)', color: 'white', padding: 'clamp(12px, 2vw, 16px) clamp(24px, 4vw, 32px)', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: 'clamp(14px, 2vw, 16px)', width: '100%', boxSizing: 'border-box', boxShadow: '0 8px 24px rgba(102,126,234,0.4)' }}>Upgrade Now 🚀</motion.a>
          </motion.div>
        </div>
      </div>
    </div>
